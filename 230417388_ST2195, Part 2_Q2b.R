...
# Counting planes in planedata appear more than once in each df
duplicates <- list()
for (year in names(dfs)) {
  duplicates[[year]] <- dfs[[year]] %>% 
    count(TailNum) %>% 
    arrange(desc(n)) %>% 
    filter(n > 1)}
...
ggplot(actuallyflew_2004,aes(x = idv_age)) +
...
ggplot(actuallyflew_2005,aes(x = idv_age)) +
  geom_histogram(aes(y = after_stat(density))) +
...
ggplot(actuallyflew_2006,aes(x = idv_age)) +
  geom_histogram(aes(y = after_stat(density))) +
...
ggplot(actuallyflew_2007,aes(x = idv_age)) +
  geom_histogram(aes(y = after_stat(density))) +
...
ggplot(actuallyflew_2008,aes(x = idv_age)) +
  geom_histogram(aes(y = after_stat(density))) +
...
ggplot(ontime_2005 %>%  filter(age > 0, age < 50), aes(x = age)) +
  geom_histogram(aes(y = after_stat(density))) +
  geom_density(color = "steelblue") +
